...
void dumpMemory(const std::string &prefix)
{
    for (unsigned i = 0; i < 2; i++) {
        ...
        /* P is dumped too: it is the PAYLOAD, which is what --fx-run loads to
         * call a real FX handler. */
...
const char *g_replayFn;
const char *g_replayWireFn;
const char *g_fxRunFn;
const char *g_fxWavFn;
bool g_fxEnergy;
TWord g_pokeAddr[256], g_pokeVal[256];
char g_pokeSpace[256];
unsigned g_nPokes;
bool g_trace;
struct Step { TWord pc; uint64_t a, b, x, y; TWord r[8]; };
std::vector<Step> g_tr[2];
...
const char *kUsage =
    ...
    "\n"
    "One FX handler on its own, out of an emulator memory dump:\n"
    "  --fx-run PID:ENTRY:R6:R7[:BLOCKS[:AMP]]  drive one per-track FX handler\n"
    "                from out/dspmem-PID-c1.bin block after block with a fresh\n"
    "                sine, reporting in/out RMS per block. All hex but BLOCKS.\n"
    "                A runaway that reproduces here bisects in sub-second\n"
    "                iterations; one that does not is an environment fault\n"
    "  --fx-wav F    the handler's stereo output per block to F, so a\n"
    "                self-oscillation's SPECTRUM can name the delay line it\n"
    "                lives in — an oscillation period is a line length\n"
    "  --fx-energy   per-stage RMS during the run: whichever buffer's energy\n"
    "                GROWS through the silent tail is the loop manufacturing it\n"
    "  --poke [X:|Y:|P:]ADDR=V  set one dumped memory word before the handler\n"
    "                runs; repeatable, ADDR hex, X: if the space is left off\n"
    "\n"
    ...
    "  --jit-vs-interp PID[:START[:END]]  run a routine from an emulator dump\n"
    "                (out/dspmem-PID-c1.bin, out/dspreg-PID-c1.bin) under the\n"
    "                JIT and again under the INTERPRETER from identical state,\n"
    "                and diff. Any divergence is a JIT bug\n"
    "  --trace       print every instruction of the interpreter pass of\n"
    "                --jit-vs-interp, and the setup loads of --probe-modulo\n";
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (s == "--jit-vs-interp") g_replayFn = val();
        else if (s == "--fx-run")       g_fxRunFn = val();
        else if (s == "--fx-energy")    g_fxEnergy = true;
        else if (s == "--fx-wav")       g_fxWavFn = val();
        else if (s == "--poke") {
            const char *a = val();
            const char *eq = strchr(a, '=');
            char space = 'x';
            if (a[1] == ':') { space = (char)tolower(a[0]); a += 2; }
            if (!eq || g_nPokes >= 256 || !strchr("xyp", space)) {
                fprintf(stderr, "octdsp: --poke wants [X:|Y:|P:]ADDR=V\n");
                return 2;
            }
            g_pokeSpace[g_nPokes] = space;
            g_pokeAddr[g_nPokes] = (TWord)strtoul(a, nullptr, 16);
            g_pokeVal[g_nPokes++] = (TWord)strtoul(eq + 1, nullptr, 0) & 0xffffff;
        }
    ...
    if (g_replayFn) {
        /*
         * JIT versus INTERPRETER on the same routine from the same state.
         *
         * The interpreter is the library's reference implementation and both
         * entry points are public, so one binary can run a stretch of the REAL
         * payload twice from byte-identical memory and registers and diff the
         * result. A divergence is a JIT bug, and it does not matter whether
         * the starting state is exactly the one the Octatrack was in — only that
         * it is the same for both runs.
         */
        ot::Core &c = g_chip.core[1];
        char path[256], pid[64];
        std::vector<TWord> mem0[3];
        snprintf(pid, sizeof pid, "%s", g_replayFn);
        if (char *cut = strchr(pid, ':'))
            *cut = 0;
        if (!strcmp(pid, "trap"))
            snprintf(path, sizeof path, "out/trap-mem.bin");
        else
            snprintf(path, sizeof path, "out/dspmem-%s-c1.bin", pid);
        FILE *f = fopen(path, "rb");
        if (!f) { fprintf(stderr, "cannot open %s\n", path); return 2; }
        const EMemArea areas[3] = {MemArea_X, MemArea_Y, MemArea_P};
        for (int i = 0; i < 3; i++) {
            mem0[i].resize(0x40000);
            if (fread(mem0[i].data(), 4, 0x40000, f) != 0x40000) {
                fprintf(stderr, "%s is short\n", path); return 2;
            }
        }
        fclose(f);
        if (!strcmp(pid, "trap"))
            snprintf(path, sizeof path, "out/trap-reg.bin");
        else
            snprintf(path, sizeof path, "out/dspreg-%s-c1.bin", pid);
        f = fopen(path, "rb");
        if (!f) { fprintf(stderr, "cannot open %s\n", path); return 2; }
        std::vector<uint8_t> regbuf(sizeof(c.dsp->regs()));
        if (fread(regbuf.data(), 1, regbuf.size(), f) != regbuf.size()) {
            fprintf(stderr, "%s is short\n", path); return 2;
        }
        fclose(f);

        TWord kStart = 0x1807, kEnd = 0x1863;
        const TWord kStart2 = 0x700, kEnd2 = 0x1870;
        if (const char *c1 = strchr(g_replayFn, ':')) {
            kStart = (TWord)strtoul(c1 + 1, nullptr, 16);
            if (const char *c2 = strchr(c1 + 1, ':'))
                kEnd = (TWord)strtoul(c2 + 1, nullptr, 16);
        }
        std::vector<TWord> outY[2];
        for (int pass = 0; pass < 2; pass++) {
            for (int i = 0; i < 3; i++)
                for (TWord a = 0; a < 0x40000; a++)
                    c.mem->set(areas[i], a, mem0[i][a] & 0xffffff);
            memcpy(&c.dsp->regs(), regbuf.data(), regbuf.size());
            auto &r = c.dsp->regs();
            if (strcmp(pid, "trap")) {
                r.r[2].var = 0xd0;        /* the descriptor the caller passes */
                r.r[4].var = 0xe0;        /* the coefficient ramp             */
                r.b.var  = 0;
            }
            c.dsp->getJit().destroyAllBlocks();
            c.dsp->setPC(kStart);
            ...
            for (unsigned n = 0; n < 200000; n++) {
                const TWord pc = c.dsp->getPC().toWord();
                /* The JIT runs a whole BLOCK per call and the interpreter one
                 * instruction, so "stop when pc == kEnd" is not a fair fence —
                 * the JIT can step over kEnd inside a block and run away. Stop
                 * when control LEAVES the routine and its helpers instead. */
                if (pc < kStart2 || pc > kEnd2) break;
                {
                    const auto &q = c.dsp->regs();
                    Step st;
                    st.pc = pc; st.a = q.a.var; st.b = q.b.var;
                    st.x = q.x.var; st.y = q.y.var;
                    for (int k = 0; k < 8; k++) st.r[k] = q.r[k].var;
                    g_tr[pass].push_back(st);
                }
                if (pass == 1 && g_trace && n < 40000) {
                    const auto &q = c.dsp->regs();
                    printf("  %04x a=%014llx b=%014llx x=%012llx y=%012llx "
                           "r4=%04x\n", pc,
                           (unsigned long long)(q.a.var & 0xffffffffffffffull),
                           (unsigned long long)(q.b.var & 0xffffffffffffffull),
                           (unsigned long long)(q.x.var & 0xffffffffffffull),
                           (unsigned long long)(q.y.var & 0xffffffffffffull),
                           q.r[4].var & 0xffff);
                }
                if (pass == 0) c.dsp->execJit(); else c.dsp->execInterpreter();
            }
            outY[pass].resize(0x20000);
            for (TWord a = 0; a < 0x10000; a++) {
                outY[pass][a] = c.mem->get(MemArea_X, a);
                outY[pass][0x10000 + a] = c.mem->get(MemArea_Y, a);
            }
        }
        /* The first instruction after which the two engines part is the one
         * to look at — everything downstream is consequence. */
        {
            const size_t n = std::min(g_tr[0].size(), g_tr[1].size());
            size_t i = 0;
            for (; i < n; i++) {
                const Step &j = g_tr[0][i], &t = g_tr[1][i];
                if (j.pc != t.pc || j.a != t.a || j.b != t.b ||
                    j.x != t.x || j.y != t.y ||
                    memcmp(j.r, t.r, sizeof j.r)) {
                    printf("FIRST DIVERGENCE at step %zu\n", i);
                    for (size_t k = (i > 3 ? i - 3 : 0); k <= i && k < n; k++) {
                        const Step &jj = g_tr[0][k], &tt = g_tr[1][k];
                        printf("  step %zu  JIT pc=%04x a=%014llx b=%014llx "
                               "x=%012llx y=%012llx\n", k, jj.pc,
                               (unsigned long long)(jj.a & 0xffffffffffffffull),
                               (unsigned long long)(jj.b & 0xffffffffffffffull),
                               (unsigned long long)(jj.x & 0xffffffffffffull),
                               (unsigned long long)(jj.y & 0xffffffffffffull));
                        printf("           INT pc=%04x a=%014llx b=%014llx "
                               "x=%012llx y=%012llx\n", tt.pc,
                               (unsigned long long)(tt.a & 0xffffffffffffffull),
                               (unsigned long long)(tt.b & 0xffffffffffffffull),
                               (unsigned long long)(tt.x & 0xffffffffffffull),
                               (unsigned long long)(tt.y & 0xffffffffffffull));
                        for (int q2 = 0; q2 < 8; q2++)
                            if (jj.r[q2] != tt.r[q2])
                                printf("           r%d JIT %06x INT %06x\n",
                                       q2, jj.r[q2], tt.r[q2]);
                    }
                    break;
                }
            ...
            if (i == n)
                printf("traces identical for %zu steps (lengths %zu/%zu)\n",
                       n, g_tr[0].size(), g_tr[1].size());
        }
        /* Compare REGISTERS too. The interpreter silently does nothing for
         * `move x:(Rn+disp),Mn` (see --probe-modulo), so a routine that sets a
         * modulo from memory diverges in M even when memory happens to match —
         * and this harness would call that "agree". */
        {
            const auto &q = c.dsp->regs();
            printf("final m4: this pass %06x\n", q.m[4].var & 0xffffff);
        }
        int bad = 0;
        for (TWord a = 0; a < 0x20000; a++) {
            if (outY[0][a] != outY[1][a]) {
                if (bad < 16)
                    printf("%c:%05x  JIT $%06x   interpreter $%06x   DIVERGES\n",
                           a < 0x10000 ? 'X' : 'Y', a & 0xffff,
                           outY[0][a], outY[1][a]);
                bad++;
            }
        }
        printf("P:%04x..%04x: %s (%d words of X+Y differ)\n", kStart, kEnd,
               bad ? "JIT AND INTERPRETER DISAGREE" : "jit and interpreter agree",
               bad);
        return bad ? 1 : 0;
    }
    if (g_fxRunFn) {
        /*
         * Drive ONE per-track FX handler block after block from an emulator
         * memory dump, with a fresh sine written into the track buffer each
         * block — the multi-block instrument the reverbs need. A runaway that
         * reproduces here is bisectable in sub-second iterations; one that
         * does not is an environment fault, not an arithmetic one.
         *
         *   --fx-run PID:ENTRY:R6:R7[:BLOCKS[:AMP]]     (all hex but BLOCKS)
         *
         * e.g. SPRING from dump 21620:  --fx-run 21620:107e:269:6200:4000
         * The dump supplies every table, parameter and state word the
         * Octatrack had; only the dispatcher's per-call registers are established here,
         * matching P:0x2f9-0x302 (n7=16, r0=x:$20e=0, a=1, b=x:$41e).
         */
        ot::Core &c = g_chip.core[1];
        char path[256], pid[64];
        snprintf(pid, sizeof pid, "%s", g_fxRunFn);
        if (char *cut = strchr(pid, ':'))
            *cut = 0;
        TWord entry = 0x107e, kr6 = 0x269, kr7 = 0x6200;
        unsigned nblocks = 4000;
        double amp = 0.1;
        {
            const char *p = strchr(g_fxRunFn, ':');
            if (p) entry = (TWord)strtoul(p + 1, nullptr, 16), p = strchr(p + 1, ':');
            if (p) kr6 = (TWord)strtoul(p + 1, nullptr, 16), p = strchr(p + 1, ':');
            if (p) kr7 = (TWord)strtoul(p + 1, nullptr, 16), p = strchr(p + 1, ':');
            if (p) nblocks = (unsigned)strtoul(p + 1, nullptr, 10), p = strchr(p + 1, ':');
            if (p) amp = atof(p + 1);
        }
        /* a PID ending in -c0/-c1 names the core; bare pids default to c1 */
        if (strstr(pid, "-c0") || strstr(pid, "-c1"))
            snprintf(path, sizeof path, "out/dspmem-%s.bin", pid);
        else
            snprintf(path, sizeof path, "out/dspmem-%s-c1.bin", pid);
        FILE *f = fopen(path, "rb");
        if (!f) { fprintf(stderr, "cannot open %s\n", path); return 2; }
        /* Load X/Y below the ICC bridge only (above it they alias into P),
         * and P only up to the payload's end. Copying the dump's whole P
         * feeds the JIT a quarter-million words of formless data and its
         * block builder spends MINUTES scanning it — sampled, not guessed.
         * Core 0's payload extends to $1fde, so the limit is $2000. */
        const EMemArea areas[3] = {MemArea_X, MemArea_Y, MemArea_P};
        const TWord lim[3] = {0x30000, 0x30000, 0x2000};
        for (int i = 0; i < 3; i++) {
            std::vector<TWord> m(0x40000);
            if (fread(m.data(), 4, 0x40000, f) != 0x40000) {
                fprintf(stderr, "%s is short\n", path); return 2;
            }
            for (TWord a = 0; a < lim[i]; a++)
                c.mem->set(areas[i], a, m[a] & 0xffffff);
        }
        fclose(f);
        /* Fence beyond-payload P with RTS. ☠ Only PopPC/branch/loop words end
         * a JIT block scan — zeros are nops, so a block starting in a zero
         * sea scans and EMITS a quarter-million words (pcMax is sizeP), which
         * takes minutes per block. An rts fence makes every stray block one
         * word, and strayed execution unwinds back through the stack. */
        for (TWord a = 0x2000; a < 0x30000; a++)
            c.mem->set(MemArea_P, a, 0x00000c);
        /* driver: jsr ENTRY, write a completion marker, then park in a
         * bset/bclr mode-change loop. Three traps live here, each hit:
         * ☠ 12-bit short jsr/jmp cannot encode these targets — the overflow
         *   lands in the opcode field and the scanner chews garbage forever;
         *   use the LONG forms.
         * ☠ pc-watching cannot detect completion: the JIT chains blocks
         *   natively across a wrap-around jmp, so control never rests at any
         *   sentinel pc. The marker WRITE (r7 — nonzero — to X:$1ffe) is the
         *   signal instead.
         * ☠ a `jmp .` park compiles to a native infinite loop; the bset/bclr
         *   pair forces a mode re-dispatch to C++ every instruction, so the
         *   host regains control without the DSP touching any state. */
        const TWord drv = 0x1f00, marker = 0x1ffe;
        /* All eight M registers are forced linear first: the payload's block
         * loop runs with them linear, but the lab boot path can leave them in
         * whatever state the payload's init parked with. */
        const TWord dw[] = { 0x05f420, 0xffffff,         /* m0..m7 linear   */
                             0x05f421, 0xffffff,
                             0x05f422, 0xffffff,
                             0x05f423, 0xffffff,
                             0x05f424, 0xffffff,
                             ...
                             0x05f426, 0xffffff,
                             0x05f427, 0xffffff,
                             0x0bf080, entry,            /* jsr >entry      */
                             0x677000, marker,           /* r7 -> x:marker  */
                             0x0af96b,                   /* bset #$b,sr     */
                             0x0af94b,                   /* bclr #$b,sr     */
                             0x0af080, drv + 20 };       /* jmp park        */
        for (unsigned i = 0; i < sizeof dw / sizeof dw[0]; i++)
            c.mem->set(MemArea_P, drv + i, dw[i]);
        for (unsigned i = 0; i < g_nPokes; i++)
            c.mem->set(g_pokeSpace[i] == 'y' ? MemArea_Y :
                       g_pokeSpace[i] == 'p' ? MemArea_P : MemArea_X,
                       g_pokeAddr[i], g_pokeVal[i]);
        const TWord bWord = c.mem->get(MemArea_X, 0x41e) & 0xffffff;
        /* --fx-wav: the mixed output (the track buffer X:$0..$1f, stereo) per
         * block, so the self-oscillation's SPECTRUM can name the loop it
         * lives in — an oscillation period is a delay-line length. */
        FILE *wavF = nullptr;
        uint64_t wavFrames = 0;
        if (g_fxWavFn) {
            wavF = fopen(g_fxWavFn, "wb");
            if (!wavF) { fprintf(stderr, "cannot write %s\n", g_fxWavFn); return 2; }
            uint8_t h[44];
            ot_wav_header(h, kRate, 2, 0);
            fwrite(h, 1, 44, wavF);
        }
        /* --fx-energy: per-stage RMS during the run. Which buffer's energy
         * GROWS during the silent tail is which loop manufactures it. The
         * regions are SPRING's (state r7, arena from x:(r7+$20)); they are
         * harmless noise for other effects. */
        const TWord arena = c.mem->get(MemArea_X, kr7 + 0x20) & 0xffffff;
        struct Probe { const char *name; char sp; TWord base; TWord len; };
        const Probe probes[] = {
            { "ring0",  'Y', arena,                    32 },
            { "ring1",  'Y', (TWord)(arena + 0x20),    32 },
            { "line1",  'Y', (TWord)(arena + 0x400), 1024 },
            { "line2",  'Y', (TWord)(arena + 0x800), 1024 },
            { "line3",  'Y', (TWord)(arena + 0xc00), 1024 },
            { "x40",    'X', 0x40, 16 },
            { "x50",    'X', 0x50, 16 },
            { "x60",    'X', 0x60, 16 },
            { "x70",    'X', 0x70, 16 },
            { "st40",   'X', (TWord)(kr7 + 0x40), 0x30 },
            { "wetY0",  'Y', 0x00, 16 },
        };
        auto &r = c.dsp->regs();
        c.dsp->getJit().destroyAllBlocks();
        double phase = 0;
        const double dph = 2 * M_PI * 440.0 / 44100.0;
        setvbuf(stdout, nullptr, _IOLBF, 0);
        printf("blk    in_rms    out_rms   out_max\n");
        for (unsigned b = 0; b < nblocks; b++) {
            double irms = 0;
            for (unsigned fno = 0; fno < 16; fno++) {
                /* burst-then-silence: input runs for the first half of the
                 * trial so the tail's decay (or self-sustainment) is visible */
                const double v = b < nblocks / 2 ? amp * sin(phase) : 0.0;
                phase += dph;
                const TWord s = (TWord)((int32_t)llround(v * 0x7fffff)) & 0xffffff;
                c.mem->set(MemArea_X, fno * 2, s);
                c.mem->set(MemArea_X, fno * 2 + 1, s);
                irms += v * v;
            }
            irms = sqrt(irms / 16);
            for (int k = 0; k < 8; k++) r.m[k].var = 0xffffff;
            r.r[0].var = 0; r.r[6].var = kr6; r.r[7].var = kr7;
            r.n[7].var = 0x10;
            r.a.var = 0x010000ull << 24;
            r.b.var = (uint64_t)bWord << 24;
            r.sr.var = 0x300;
            c.mem->set(MemArea_X, marker, 0);
            c.dsp->setPC(drv);
            unsigned n = 0;
            for (; n < 400000; n++) {
                c.dsp->execJit();
                if (c.mem->get(MemArea_X, marker) & 0xffffff) break;
                if (n && (n % 50000) == 0)
                    printf("blk %u: still running, pc=%06x after %u exec\n",
                           b, c.dsp->getPC().toWord(), n);
            }
            if (n == 400000) {
                printf("blk %u: WEDGED at pc=%06x\n", b, c.dsp->getPC().toWord());
                return 1;
            }
            double orms = 0; double omax = 0;
            for (unsigned i = 0; i < 32; i++) {
                TWord v = c.mem->get(MemArea_X, i) & 0xffffff;
                double s = (v & 0x800000 ? (double)v - 0x1000000 : (double)v) / 0x800000;
                orms += s * s;
                if (fabs(s) > omax) omax = fabs(s);
            }
            orms = sqrt(orms / 32);
            if (wavF) {
                int16_t fr[32];
                for (unsigned i = 0; i < 32; i++) {
                    TWord v = c.mem->get(MemArea_X, i) & 0xffffff;
                    int32_t s = (int32_t)(v << 8) >> 8;
                    fr[i] = (int16_t)(s >> 8);
                }
                fwrite(fr, 2, 32, wavF);
                wavFrames += 16;
            }
            if (b < 8 || (b & (b < 256 ? 0x1f : 0xff)) == 0 || b + 1 == nblocks)
                printf("%-6u %.6f  %.6f  %.6f\n", b, irms, orms, omax);
            if (g_fxEnergy && (b & 0x3f) == 0) {
                printf("E %-6u", b);
                for (const auto &p : probes) {
                    double e = 0;
                    for (TWord i = 0; i < p.len; i++) {
                        const TWord v = c.mem->get(
                            p.sp == 'X' ? MemArea_X : MemArea_Y,
                            p.base + i) & 0xffffff;
                        const double s = (v & 0x800000 ? (double)v - 0x1000000
                                                       : (double)v) / 0x800000;
                        e += s * s;
                    }
                    printf(" %s=%.5f", p.name, sqrt(e / p.len));
                }
                printf("\n");
            }
        }
        if (wavF) {
            ot_wav_patch(wavF, (uint32_t)wavFrames, 2);
            fclose(wavF);
        }
        if (!g_dumpMem.empty())
