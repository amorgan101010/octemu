...
| usb-midi.s — the glue that resurrects the firmware's dormant USB-MIDI half.
|
| Linked into the in-image free zone by custom/usb-midi.py. Five shims,
| a USB-MIDI RX decoder, and the grown config descriptors. The TX encoder,
| EP2 bring-up and EP2 primitives are the firmware's own dormant code
| (usb_midi_* in re/coldfire.syms) — this file only wires them in.
|
| Hooks (each a 6-byte jmp planted by the patch, displaced bytes replayed):
